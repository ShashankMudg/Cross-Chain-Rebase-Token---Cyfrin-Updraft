...
contract RebaseTokenPool is TokenPool {
        ...
        returns (Pool.LockOrBurnOutV1 memory lockOrBurnOut)
    {
        ...
        uint256 userInterestRate = IRebase_Token(address(i_token))._getInterestRate(lockOrBurnIn.originalSender);
        //uint256 currentInterestRate = IRebaseToken(address(i_token)).getInterestRate();
        IRebase_Token(address(i_token)).burn(address(this), lockOrBurnIn.amount);
        ...
        returns (Pool.ReleaseOrMintOutV1 memory)
    {
        ...
        IRebase_Token(address(i_token)).mint(receiver, releaseOrMintIn.amount, userInterestRate);
